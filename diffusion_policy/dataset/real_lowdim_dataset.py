...
from diffusion_policy.real_world.real_data_conversion import real_lowdim_data_to_replay_buffer
...
class RealLowdimDataset(BaseLowdimDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    with threadpool_limits(1):
        replay_buffer = real_lowdim_data_to_replay_buffer(
            dataset_path=dataset_path,
            out_store=store,
...
def test():
    ...
    with hydra.initialize('../config'):
        cfg = hydra.compose('train_diffusion_unet_real_lowdim_workspace.yaml')
